...
function handleSelectLang(e){
  // console.log("language ko select karo",e.target.value);
    ...
    <div className='absolute w-full bg-gradient-to-b from-black px-8 py-2 z-20 flex items-center justify-between'>
        <img className='w-44  '
         ...
         {user && <div className='flex gap-2'>
           {showGptSearch ? <select className='bg-amber-50 text-black' onChange={handleSelectLang}>
              {  select_language.map((el)=><option value={el.identifier} >{el.lang}</option>)}  
              </select>:null}
            <button className='px-4 py-2 bg-red-700 rounded-lg text-amber-50 relative z-20' onClick={handleGptToggle}>{showGptSearch ?<div>Home</div>:<div>GPT Search</div>}</button>
